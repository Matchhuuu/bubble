...
<head>
    ...
    <title>Current Inventory Stocks</title>
</head>
<script>
    function myFunction() {
...
<div class="scroll">
    <table class="table table-bordered table-striped">
        ...
        $sql_returned_goods = "SELECT ITEM_ID,
                                        ITEM_NAME,
                                        CATEGORY,
                                        UNIT,
                                        CURRENT_QUANTITY,
        ...
        while ($row = $result_returned_goods->fetch_assoc()) {
            ...
            if (($row['is_liquid'] != 1 && $row['CURRENT_QUANTITY'] > 50) || 
                ...
                $liquidStatus = $row['is_liquid'] == 1 ? 'Yes' : 'No';
                echo "
                <tr>
                    <td>{$row['ITEM_ID']}</td>
                    <td>{$row['ITEM_NAME']}</td>
...
<script>
    ...
    document.querySelectorAll('table tr').forEach(row => {
        ...
        const stockCell = cells[3]; // 4th column (Stock Quantity)

        if (stockCell) {
            const value = parseFloat(stockCell.innerText);
            const isLiquid = stockCell.getAttribute('data-liquid') === '1';
            
            if (!isNaN(value)) {
                // Color coding for liquid stocks (red ≤300, yellow ≤800)
                if (isLiquid) {
                    if (value <= 300) {
                        stockCell.style.backgroundColor = 'red';
                        stockCell.style.color = 'white';
                    } else if (value <= 800) {
                        stockCell.style.backgroundColor = 'yellow';
                        stockCell.style.color = 'black';
                    } else {
                        stockCell.style.backgroundColor = 'transparent';
                    }
                } 
                // Color coding for non-liquid stocks (red ≤10, yellow ≤50)
                else {
                    if (value <= 10) {
                        stockCell.style.backgroundColor = 'red';
                        stockCell.style.color = 'white';
