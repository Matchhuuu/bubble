...
{
    return 'TBL1-' . str_pad(rand(1, 9999), 4, '0', STR_PAD_LEFT);
}
...
function addCustomerOrderRecord($orderId, $orderType, $total, $discount = 0, $status = 'pending')
{
    ...
    if ($columnCheck && $columnCheck->num_rows === 0) {
        $conn->query("ALTER TABLE customer_orders ADD COLUMN amount_paid DECIMAL(10,2) DEFAULT 0 AFTER discount");
    }
    $stmt = $conn->prepare("INSERT INTO customer_orders (order_id, order_type, total, discount, amount_paid, status) VALUES (?, ?, ?, ?, ?, ?)");
    if (!$stmt) {
        throw new Exception("Prepare failed: " . $conn->error);
    }
    $amountPaid = 0;
    $stmt->bind_param("ssddds", $orderId, $orderType, $total, $discount, $amountPaid, $status);
    if (!$stmt->execute()) {
