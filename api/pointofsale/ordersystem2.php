...
{
    return 'TBL2-' . str_pad(rand(1, 9999), 4, '0', STR_PAD_LEFT);
}
...
function deductIngredients($menuItemId, $sizeId, $quantity) {
    ...
    try {
        ...
        $query = "SELECT ums.ingredient_id, ums.quantity_required, ui.item_id, ui.item_name, 
                         ui.current_quantity, ui.unit, ui.cost_per_unit
                  FROM unified_menu_system ums
                  JOIN unified_inventory ui ON ums.ingredient_id = ui.item_id
                  WHERE ums.menu_item_id = ? AND ums.size_id = ? AND ums.ingredient_id IS NOT NULL";
        ...
        foreach ($ingredients as $ing) {
            ...
            $current = floatval($ing['current_quantity']);
        ...
        foreach ($ingredients as $ing) {
            ...
            $update->bind_param("di", $deduct, $ingredientId);
            if (!$update->execute()) {
            ...
            $insertHistory = $conn->prepare(
                ...
                (PROD_ID, PROD_NAME, QTY_ORDERED, UNIT_ORDERED, STOCK_PER_UNIT, QTY_DEPLETED, BASE_UNIT, TOT_PRICE, DATE_DEP, TIME_DEP, CREATED_AT) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())"
            );
            ...
            if (!$insertHistory->execute()) {
                throw new Exception("Failed to insert depletion history: " . $insertHistory->error);
            }
            $insertHistory->close();
        }
        ...
        $conn->rollback();
        error_log("deductIngredients Error: " . $e->getMessage());
        throw $e;
    }
}
    ...
    if (isset($menu[$section][$category][$item][$size])) {
        ...
        if (checkStockAvailability($menu_item_id, $size, 1)) {
            ...
            if ($item_index !== false) {
                ...
                } else echo "<script>alert('Sorry, not enough stock available for " . addslashes($item) . "');</script>";
            } else {
                $_SESSION['cart'][] = ['id'=>$menu_item_id,'name'=>$item,'size'=>$size,'price'=>$price,'quantity'=>1,'total_price'=>$price,'flavor'=>$flavor];
            }
        } else echo "<script>alert('Sorry, " . addslashes($item) . " is currently out of stock.');</script>";
    }
}
...
if (isset($_POST['decrease_quantity'])) {
    ...
    if (isset($_SESSION['cart'][$i])) {
        if ($_SESSION['cart'][$i]['quantity'] > 1) {
            $_SESSION['cart'][$i]['quantity']--;
            $_SESSION['cart'][$i]['total_price'] -= $_SESSION['cart'][$i]['price'];
        } else { unset($_SESSION['cart'][$i]); $_SESSION['cart'] = array_values($_SESSION['cart']); }
    }
}
...
foreach ($_SESSION['cart'] as $item) $total += $item['total_price'];
