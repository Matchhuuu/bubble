...
// Get total count
$count_query = "SELECT COUNT(*) as total FROM customer_orders $where_clause";
...
// Get orders
$query = "SELECT * FROM customer_orders $where_clause ORDER BY created_at DESC LIMIT ? OFFSET ?";
$params[] = $limit;
$params[] = $offset;
$types .= 'ii';
...
<body>
    <div class="nav-header">
        <a href="/bubble/interface/admin_homepage.php" class="back-button">← Back</a>
        <h2>Order History</h2>
        <a href="/bubble/pointofsale/order_archive.php" class="back-button"> Order Archive</a>
    </div>
    ...
    <div class="container">
        ...
        <div class="search-filters">
            <form method="GET">
                ...
                <button class="btn-aa" type="submit">Search</button>
                <a href="new_receipt_records.php">Clear</a>
...
<div class="a">
    <div class="dropdown-ver" id="exportModal">
        <div class="body" >
            ...
            <form id="exportForm" method="POST" class="export-from">
                <div class="form-a">
        ...
        <br><br>
        <div class="b">
            <button type="button" class="aa" onclick="closeExportModal()">&nbsp; Close &nbsp;</button>
            <button type="submit" class="aa" form="exportForm" formaction="export_pdf.php" class="btn-edit">&nbsp; Export PDF &nbsp;</button>
            <button type="submit" class="aa" form="exportForm" formaction="export_excel.php" class="btn-edit">&nbsp; Export Excel &nbsp;</button>
        </div>
    </div>
</div>
 ...
 <!--                                <td>₱<?php echo number_format($order['discount'], 2); ?></td>
                                ...
                                <td>
                                    <a href="view_receipt_for_admin.php?order_id=<?php echo urlencode($order['order_id']); ?>" 
    ...
    fetch("get_available_dates.php")
        ...
        .then(data => {
            ...
            data.months.forEach(month => {
                const date = new Date(month + "-01"); // convert "YYYY-MM" → Date
                const formatted = date.toLocaleString("en-US", { month: "long", year: "numeric" });
                const opt = document.createElement("option");
                opt.value = month; // still send "YYYY-MM" to backend
                opt.text = formatted;
            ...
            data.days.forEach(day => {
                const date = new Date(day); // convert "YYYY-MM-DD" → Date
                const formatted = date.toLocaleString("en-US", { month: "long", day: "numeric", year: "numeric" });
                const opt = document.createElement("option");
                opt.value = day; // still send "YYYY-MM-DD" to backend
                opt.text = formatted;
