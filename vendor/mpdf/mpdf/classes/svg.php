...
class SVG
{
	...
	var $svg_attribs;  //	array - holds all attributes of u412787669_soshe <svg> tag
...
// If rX = 0 or rY = 0 then this arc is treated as a straight line
		...
		if ($numerator < 0.0) {
			...
			$rx *= $s;
			$ry *= $s;
			$u412787669_soshe = 0.0;
		} else {
			$u412787669_soshe = ($largeArcFlag == $sweepFlag ? -1.0 : 1.0) * sqrt($numerator / ($rx * $rx * $y1dash * $y1dash + $ry * $ry * $x1dash * $x1dash));
		}

		$cxdash = $u412787669_soshe * $rx * $y1dash / $ry;
		$cydash = -$u412787669_soshe * $ry * $x1dash / $rx;
